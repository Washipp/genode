...
/* Make the process sleep for SECONDS seconds, or until a signal arrives
   ...
   __THROW.  */
extern "C" unsigned int sleep (unsigned int _seconds);
...
/* Sleep USECONDS microseconds, or until a signal arrives that is not blocked
   ...
   This function is a cancellation point and therefore not marked with
   __THROW.  */
extern "C" int usleep (__useconds_t _useconds);
...
extern "C" __off_t lseek (int _fd, __off_t _offset, int _whence);

/* Remove the link NAME.  */
extern "C" int unlink (const char *_name);

/* Truncate the file FD is open on to LENGTH bytes.  */
extern "C" int ftruncate (int _fd, __off_t _length);

/* Create a new session with the calling process as its leader.
   The process group IDs of the session and the calling process
   are set to the process ID of the calling process, which is returned.  */
extern "C" int setsid (void);
...
extern "C" long int read (int fd, void *buf, size_t nbytes);

/* Write N bytes of BUF to FD.  Return the number written, or -1.*/
extern "C" long int write (int fd, const void *buf, size_t n);
...
extern "C" int pipe (int pipedes[2]);


/* Make a symbolic link to FROM named TO.  */
extern "C" int symlink (const char *from, const char *to);

/* Get the process ID of the calling process.  */
extern "C" int getpid (void);

/* Get the process group ID of process PID.  */
extern "C" int getpgid (int pid);
...
extern "C" long int syscall (long int sysno, ...);

/* Get the value of the system variable NAME.  */
extern "C" long int sysconf (int name);
